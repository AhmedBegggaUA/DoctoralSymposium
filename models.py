...
class DJ(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels,num_centers,adj_dim, n_jumps,out_channels,drop_out = 0.5):
        ...
        self.classify1 = Linear(num_centers, out_channels)
        #GNN
        ...
        # Aux
        #self.mlpW = Linear(hidden_channels + num_centers, hidden_channels)
        self.mlpFinal = Linear(hidden_channels + num_centers, out_channels)
        #Aux
        self.drop_out = drop_out
        self.new_adj = None
        self.embeddings = None
    ...
    def forward(self, x, adj):
        xX = self.mlpx(x)
        xA = self.MLP(adj)
        xA, pump_loss, ortho_loss,_= pump(adj, xA)
        xA = xA.squeeze(0)
